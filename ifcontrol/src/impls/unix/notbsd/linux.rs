use ifstructs::{ethtool_drvinfo, ETHTOOL_GDRVINFO, ifreq, rtentry};
use ifstructs::get_name;
use libc;
...
pub fn add_iface_to_bridge<F: AsRawFd>(
    ...
    let iface_idx = crate::impls::get_iface_index(ctl_fd, iface_ifname)?;
...
pub fn get_iface_name<F: AsRawFd>(ctl_fd: &F, idx: libc::c_int) -> Result<String, IfError> {
    ...
    unsafe { crate::impls::ioctl_get_iface_name(ctl_fd.as_raw_fd(), &mut req)? };
    Ok(req.get_name()?)
}

pub fn get_iface_index<F: AsRawFd>(ctl_fd: &F, ifname: &str) -> Result<libc::c_int, IfError> {
    let mut req = ifreq::from_name(ifname)?;
    unsafe { crate::impls::ioctl_get_iface_index(ctl_fd.as_raw_fd(), &mut req)? };
...
pub fn get_ethernet_driver<F: AsRawFd>(ctl_fd: &F, ifname: &str) -> Result<DriverInfo, IfError> {
    ...
    unsafe { crate::impls::ioctl_ethtool(ctl_fd.as_raw_fd(), &mut req)? };
    Ok(DriverInfo {
...
mod tests {
    ...
    // #[test]
    // fn test_bridge() {
    //     let ctl_fd = crate::impls::new_control_socket().unwrap();
